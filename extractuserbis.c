...
void find( int nbref, int idref,int *listref ,FILE *f){
  ...
  LISTE2=fopen("vu.txt","r");

    int nbmanga2;
    //printf ("Etape -A\n");
    
    fscanf(LISTE2,"%d",&nbmanga2);

    //if (ident == searchid) search = i;
    int *listban = malloc(sizeof(int) * nbmanga2);
    for ( int k = 0 ; k < nbmanga2; k++){
      fscanf(LISTE2,"%d",&listban[k]);
    }
qsort(listban, nbmanga2, sizeof(int), cmpfunc);
  ...
  for(int i = 0; i<number;i++){
    for(int k = 0; k<receuil[i].nb; k++){
      ...
      if ((!nbjoin( 1,&receuil[i].liste[k], nbref,listref ) && !nbjoin( 1,&receuil[i].liste[k], nbmanga2,listban )))
	{
	//printf("res %d = %d\n",receuil[i].liste[k],(nbjoin( 1,&receuil[i].liste[k], nbmanga2,listban )));
...
printf("%d\n", nbmanga2) ;
  for (int i = 0; i < nbres ; i++){
	int stock = nbjoin( 1,&tab2[i].a, nbmanga2,listban );
    fprintf(f, "; %d(%duser(%d))",tab2[i].a,tab2[i].count,stock );
  }
  fprintf(f, "\n");  
  //printf("Val = %d sur %d\n", 1, total);
